...
class Model:
    ...
    def spikes(self, N, M, R, Rs, generatorType: RandomGeneratorType, data=None, hardcode=1):
        ...
        negative_spikes = np.fromiter(
            generator.generate(a=-hardcode * R - Rs, b=-hardcode * R + Rs, N=M - positive_spikes_len), float)
    ...
    def _cast_to_interval(cls, data, R, shift_constant=0.5, multiply_constant=2, add_constant=0):
        ...
        data_interval = ((data - data_min) / (
                data_max - data_min) - shift_constant) * multiply_constant * R + add_constant
